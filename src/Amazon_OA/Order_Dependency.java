...
public class Order_Dependency {
    ...
    public static List<Order> mySolution(List<OrderDependency> orderDependencyList) {
        if (orderDependencyList == null || orderDependencyList.size() == 0) {
            return new ArrayList<>(0);
        }
        Map<String, Integer> map = new HashMap<>();
        Set<String> set = new HashSet<>();
        for (OrderDependency orderDependency : orderDependencyList) {
            set.add(orderDependency.cur.orderName);
            set.add(orderDependency.pre.orderName);
            if (!map.containsKey(orderDependency.cur.orderName)) {
                map.put(orderDependency.cur.orderName, 1);
            } else {
                map.put(orderDependency.cur.orderName, map.get(orderDependency.cur.orderName) + 1);
            }
        }
        ...
        List<Order> result = new ArrayList<>(set.size());
        while (!queue.isEmpty()) {
            String order = queue.poll();
            result.add(new Order(order));
            for (OrderDependency orderDependency : orderDependencyList) {
                if (orderDependency.pre.orderName.equals(order)) {
                    map.put(orderDependency.cur.orderName, map.get(orderDependency.cur.orderName) - 1);
                    if (map.get(orderDependency.cur.orderName) == 0) {
                        queue.offer(orderDependency.cur.orderName);
                    }
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        List<Order> myRes = mySolution(list);
