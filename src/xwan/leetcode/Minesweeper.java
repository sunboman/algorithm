 ...
 * Let's play the minesweeper game (Wikipedia, online game)!
 ...
 'B' represents a revealed blank square that has no adjacent (above, below, left, right, and all 4 diagonals) mines, digit ('1' to '8') represents how many mines are adjacent to this revealed square,
...
public class Minesweeper {
    ...
    private static void dfs(char[][] board, int i, int j) {
        if (!isValid(board, i, j) || board[i][j] == 'B') return;
        int numOfBombs = getBombs(board, i, j);
        ...
        if (numOfBombs == 0) {
            ...
            board[i][j] = (char) (numOfBombs + '0');
        }
    }

    private static int getBombs(char[][] board, int i, int j) {
        int count = 0;
        for (int d = 0; d < 8; d++) {
            int x = i + dirs[d][0];
            int y = j + dirs[d][1];
