...
that k-1 persons are skipped and kth person is killed in circle. The task is to choose the place in the initial circle so
that you are the last one remaining and so survive.

For example, if n = 5 and k = 2, then the safe position is 3. Firstly, the person at position 2 is killed, then person
at position 4 is killed, then person at position 1 is killed. Finally, the person at position 5 is killed. So the person
at position 3 survives.
If n = 7 and k = 3, then the safe position is 4. The persons at positions 3, 6, 2, 7, 5, 1 are killed in order, and
...
public class Josephus_Ring {
    public int getLast(int n, int k) {
        ...
        return (getLast(n - 1, k) + k - 1) % n + 1;
    }
    public int getLastII(int n, int k) {
        int s = 0;
        for (int i = 2; i <= n; i++) {
            s = (s + k) % i;
        }
    ...
    public static void main(String[] args) {
        int res = new Josephus_Ring().getLastII(5, 2);
    }
}
